...
## Author & review note (reading this for grading)

This report and the code were drafted with substantial assistance from an AI coding agent. Before submission the student should **read this document, re-verify the numbers against `three_class_results.json`, and rework the framing and conclusions into their own words** — the agent's interpretation of *why* the model behaves this way is offered as a starting point, not as a substitute for the student's own analysis. All results here trace to the saved run output in this repository, so any number can be independently checked.
